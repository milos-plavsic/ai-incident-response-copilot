...
def _maybe_llm_enrich(incident: str, report: IncidentReport) -> IncidentReport:
    try:
        from agent_core import complete_text, llm_available

        if not llm_available():
            return report
        enriched = complete_text(
            f"Incident: {incident}\n\nPattern: {report.pattern_name}\n"
            f"Draft hypothesis: {report.hypothesis}\n\n"
            "Return a sharper root-cause hypothesis in 2 sentences.",
            system="You are an SRE incident commander.",
        )
        steps = complete_text(
            f"Given hypothesis: {enriched}\nList 4 numbered remediation steps.",
            system="You are an SRE incident commander.",
            max_tokens=400,
        )
        extra_steps = [s.strip() for s in steps.split("\n") if s.strip()][:4]
        return IncidentReport(
            pattern_name=report.pattern_name,
            hypothesis=enriched,
            confidence=min(1.0, report.confidence + 0.05),
            severity=report.severity,
            remediation_steps=extra_steps or report.remediation_steps,
...
class IncidentAnalyzer:
    ...
    def analyze(self, incident: str) -> IncidentReport:
        ...
        return _maybe_llm_enrich(incident, report)
